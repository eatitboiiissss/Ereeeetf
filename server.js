...
CREATE TABLE IF NOT EXISTS announcements(id TEXT PRIMARY KEY,text TEXT NOT NULL,created_by TEXT NOT NULL,expires_at INTEGER,active INTEGER DEFAULT 1,color TEXT DEFAULT '#f04747',bg_color TEXT DEFAULT '#000000',created_at INTEGER DEFAULT(unixepoch()));
...
safeAddColumn('announcements', 'color', "TEXT DEFAULT '#f04747'");
safeAddColumn('announcements', 'bg_color', "TEXT DEFAULT '#000000'");
...
app.get('/api/friends', auth, (req, res) => {
  const uid = req.user.id;
  const rows = db.prepare('SELECT f.*,u.id as uid,u.username,u.display_name,u.avatar,u.avatar_emoji,u.status as user_status,u.custom_status FROM friendships f JOIN users u ON(CASE WHEN f.requester_id=? THEN f.addressee_id ELSE f.requester_id END)=u.id WHERE f.requester_id=? OR f.addressee_id=? ORDER BY f.created_at DESC').all(uid, uid, uid);
  res.json(rows.map(f => ({ friendshipId: f.id, status: f.status, isRequester: f.requester_id === uid, user: { id: f.uid, username: f.username, displayName: f.display_name, avatar: f.avatar, avatarEmoji: f.avatar_emoji, status: f.user_status, customStatus: f.custom_status } })));
});
app.post('/api/friends/request', auth, (req, res) => {
  ...
  res.json({ ok: true, friendshipId: id, friendship: { friendshipId: id, status: 'pending', isRequester: true, user: su(target) } });
});
app.post('/api/friends/:id/accept', auth, (req, res) => {
  ...
  const acceptor = db.prepare('SELECT * FROM users WHERE id=?').get(req.user.id);
  const requester = db.prepare('SELECT * FROM users WHERE id=?').get(f.requester_id);
  // Tell requester: their request was accepted — send them the acceptor as the friend
  sendToUser(f.requester_id, {
    type: 'FRIEND_ACCEPTED',
    friendshipId: req.params.id,
    user: su(acceptor),
    friendship: { friendshipId: req.params.id, status: 'accepted', isRequester: true, user: su(acceptor) }
  });
  // Tell acceptor: confirm acceptance on their own screen — send them the requester as the friend
  sendToUser(req.user.id, {
    type: 'FRIEND_ACCEPTED_SELF',
    friendshipId: req.params.id,
    user: su(requester),
    friendship: { friendshipId: req.params.id, status: 'accepted', isRequester: false, user: su(requester) }
  });
  res.json({ ok: true });
});
...
app.post('/api/admin/announce', auth, (req, res) => {
  if (!isOwner(req.user.username)) return res.status(403).json({ error: 'Owner only' });
  const { text, durationMinutes, color, bgColor } = req.body;
  if (!text?.trim()) return res.status(400).json({ error: 'Text required' });
  db.prepare('UPDATE announcements SET active=0').run();
  const exp = durationMinutes ? Math.floor(Date.now()/1000) + (parseInt(durationMinutes)*60) : null;
  const id = uuidv4();
  const textColor = color || '#f04747';
  const backgroundColor = bgColor || '#000000';
  db.prepare('INSERT INTO announcements(id,text,created_by,expires_at,active,color,bg_color) VALUES(?,?,?,?,1,?,?)').run(id, text.trim(), req.user.username, exp, textColor, backgroundColor);
  const ann = { id, text: text.trim(), createdBy: req.user.username, expiresAt: exp, color: textColor, bgColor: backgroundColor };
...
app.get('/api/announce/active', (req, res) => {
  ...
  res.json({ ...a, color: a.color || '#f04747', bgColor: a.bg_color || '#000000' });
});
